 ...
 *   git archive HEAD | tar -x -C ../espresso-live
 */
'use strict';
const fs = require('fs');
...
<meta name="description" content="Every espresso machine guide, comparison and how-to on ${SITE.name} — research-based, honest, and updated for 2026.">
...
/* This site publishes NO ratings: Amazon's API returns no rating/reviewCount for these
   machines, so any star, score, review count or paraphrased review sentiment is fabricated.
   These three checks enforce that mechanically — a prompt is not a guarantee. */
const sentiment = /owners? (report|say|note|mention|describe|call|keep)|reviewers? (consistently |repeatedly )?(note|say|report|mention|describe|call)|buyers (say|report|note)|according to [\d,]+ (verified )?reviews|the theme that repeats across owner reviews|review(er)?s praise|customers (say|report)/i;
const ratingTok = /★|\b\d(?:\.\d)?\s*(?:out of 5|\/\s*5)\b|\b\d\.\d\s*(?:stars?|rating)\b|\b[\d,]{3,}\s*(?:verified\s*)?(?:owner\s*|customer\s*)?(?:ratings|reviews)\b|"?aggregateRating"?/i;
/* "reviews" is legitimate in prose that points readers AT the retailer's reviews, so the
   sentiment/rating checks are deliberately narrow: they fire on claims, not on references. */
...
for (const f of htmlFiles) {
  ...
  const s = h.match(sentiment); if (s) probs.push('REVIEW SENTIMENT (no review data exists): "' + s[0] + '"');
  const r = h.match(ratingTok); if (r) probs.push('RATING TOKEN (unverifiable): "' + r[0] + '"');
